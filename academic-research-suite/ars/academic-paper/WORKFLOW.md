...
A general-purpose academic paper writing tool — 12-agent pipeline covering all disciplines, with geography as the default reference, especially human geography, geographic information science (GIScience), and health geography.
...
**Minimal command:**
```
Write a paper on spatial inequality in urban health service access
```
...
- Domain: `geography_domain_glossary` (bilingual), `journal_submission_guide`, `latex_template_reference`
...
Default output is bilingual English + Simplified Chinese unless the user requests otherwise. Academic terminology is kept in English where appropriate. Bilingual abstracts are always provided regardless of the main text language.
...
```
academic-paper + geospatial data/tools -> Evidence-based geography paper with spatial data, GIS methods, and health/place analysis
