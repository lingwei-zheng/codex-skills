...
name: ARS
description: >
  ARS, a Codex-native Academic Research Skills suite for deep research, academic paper
  ...
  Default language support is English and Simplified Chinese. Default domain
  focus is geography, human geography, GIScience, and health geography.
---
...
## Local Domain And Language Defaults

This install is customized for geography, human geography, GIScience, and health
geography. When the user has not supplied a field frame, interpret research
questions, method choices, venue fit, and writing advice through that domain
context.

Default output language is English plus Simplified Chinese when bilingual output
is useful. If the upstream ARS wording asks for Traditional Chinese, use
Simplified Chinese instead unless the user explicitly requests Traditional
Chinese. Chinese working notes, reports, prompts, and review outputs should
therefore default to Simplified Chinese.

Use the local NORA-derived reference pack in
`../shared/field-context/` only when domain grounding is needed. Do not treat
NORA as an installed or callable skill. Recommended field files:

- `academic-writing.md` for geography/GIScience writing expectations.
- `spatial-methods.md` for spatial analysis, scale, units, and validity checks.
- `geoai-domain.md` for GeoAI and geospatial modeling context.
- `environmental-health.md` for health geography and environmental exposure logic.
- `literature-mining.md` and `research-iteration.md` for literature gap and idea refinement.
- `journal-templates/` for GIScience, remote sensing, and geoscience venue fit.
...
- For Chinese, use Simplified Chinese unless the user explicitly requests Traditional Chinese.
