...
> **For agentic workers:** Implement this plan task-by-task, using a fresh worker per task when delegation is available or inline execution otherwise. Steps use checkbox (`- [ ]`) syntax for tracking.
...
**2. Inline Execution** — execute tasks sequentially in the current session, batching execution with human checkpoints every 2-3 tasks.
