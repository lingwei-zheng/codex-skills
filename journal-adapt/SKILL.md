...
>    (1) Geography / GIScience / Health Geography — bundled default for Lingwei's research areas (`base_rules/geography_giscience_health.md`)
>    (2) General academic — bundled fallback for manuscripts outside the geography/GIScience/health-geography scope (`base_rules/general_academic.md`)
>    (3) Economics — bundled lightweight economics defaults (`base_rules/economics.md`)
>    (4) ML / CV / NLP — bundled lightweight AI conference defaults (`base_rules/ml_cv_nlp.md`)
>    (5) CS / Engineering — bundled lightweight technical writing defaults (`base_rules/cs_engineering.md`)
>    (6) External economics skill — `hanlulong/econ-writing-skill`
>    (7) External ML/CV/NLP skill — `Master-cai/Research-Paper-Writing-Skills`
>    (8) External research paper skill — `SNL-UCSB/paper-writing-skill`
>    (9) External philosophy/interdisciplinary skill — `lishix520/academic-paper-skills`
>    (10) External generic humanizer — `blader/humanizer`
>    (11) Custom file — provide a path to your own `SKILL.md` or writing guide
>    (12) None — rely only on corpus-derived dynamic rules
...
- (1) -> `base_rules/geography_giscience_health.md`
- (2) -> `base_rules/general_academic.md`
- (3) -> `base_rules/economics.md`
- (4) -> `base_rules/ml_cv_nlp.md`
- (5) -> `base_rules/cs_engineering.md`

For external skills (6)-(10), explain that they are recommendations, not dependencies. Ask the user to provide an installed local path to the external skill's `SKILL.md` or choose another option. Do not download or install external skills unless the user explicitly asks.

If user selects (11), ask for the file path and load that file as the static base layer.

If user selects (12), skip the static base layer. Only corpus-derived rules and cleanup rules will apply.

If the user is unsure and the manuscript concerns geography, human geography, GIScience, GeoAI, remote sensing, spatial data science, health geography, or environmental health, recommend (1) Geography / GIScience / Health Geography. Otherwise recommend (2) General academic. For more detail, refer to `docs/STATIC_SKILL_RECOMMENDATIONS.md`.
...
|-----------|------------|
| Geography / GIScience / Health Geography | `base_rules/geography_giscience_health.md` |
